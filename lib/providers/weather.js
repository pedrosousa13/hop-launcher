...
function buildWeatherRow(payload, key, stale = false) {
    const summary = payload.conditionText || 'Unknown';
    const icon = payload.icon || '';
    const temp = payload.tempText || 'n/a';
    const wind = payload.windText || 'n/a';
    const place = payload.placeLabel || payload.location;
    const updated = payload.observedAt || new Date().toLocaleTimeString();
    const staleHint = stale ? ' (stale)' : '';
    const primary = `${icon ? `${icon} ` : ''}${temp} • ${summary}`.trim();
    const secondary = `${place} • Wind ${wind} • Updated ${updated}${staleHint}`;
    ...
    return {
        ...
        copyText: `${place}: ${primary} (wind ${wind})`,
        execute: () => {},
    };
}
...
export class WeatherProvider {
    ...
    async _fetchWeather(location) {
        ...
        const [placeLabel, icon, tempText, windText, conditionText] = line.split('|').map(part => normalize(part));
        if (!placeLabel || !tempText)
    ...
    getResults(query, mode = 'all') {
        ...
        if (!stale && cached) {
            ...
            return [buildErrorRow(parsed.location, key, false)];
        }
        ...
        if (cached) {
            ...
            return [buildErrorRow(parsed.location, key, true)];
        }
