...
function computeFuzzyScorePrepared(queryLower, candidateRaw, candidateLower) {
    ...
    if (qIndex !== q.length) {
        ...
        // Keep single-typo queries discoverable without letting unrelated
        // candidates through at the default fuzzy threshold.
        return alt > 0 ? alt - 8 : Number.NEGATIVE_INFINITY;
    }

    const span = firstMatch >= 0 ? (lastMatch - firstMatch + 1) : 0;
    const dispersion = Math.max(0, span - q.length);
    if (q.length >= 4)
        score -= dispersion * 4.0;
    else if (q.length === 3)
        score -= dispersion * 1.6;
