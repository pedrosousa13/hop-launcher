...
configure_gnome_shortcut() {
  ...
  if ! command -v gsettings >/dev/null 2>&1; then
    echo "GNOME detected but gsettings is unavailable; skipping shortcut auto-setup." >&2
    return
  fi
...
configure_kde_shortcut() {
  ...
  if command -v qdbus6 >/dev/null 2>&1 || command -v qdbus >/dev/null 2>&1; then
    ...
    echo "KDE detected but qdbus/qdbus6 is missing; configure manually:"
    echo "  ${trigger_cmd}"
  fi
}
...
if [[ ! -f "${HOPD_DIR}/Cargo.toml" || ! -f "${HOTKEYD_DIR}/Cargo.toml" ]]; then
  echo "Could not find crates/hopd or crates/hop-hotkeyd under: ${ROOT_DIR}" >&2
  exit 1
fi
...
if [[ "$ENABLE_SERVICE" -eq 1 ]]; then
  ...
  echo "Installed units but did not enable/start them (--no-enable)."
fi

echo "hopd installed at ${INSTALL_BIN_PATH}"
echo "hop-hotkeyd installed at ${HOTKEYD_BIN_PATH}"
echo "hopd socket path: ${SOCKET_PATH}"
