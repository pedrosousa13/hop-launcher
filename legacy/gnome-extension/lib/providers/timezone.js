...
function runWithTimeout(run, timeoutMs) {
    return new Promise((resolve, reject) => {
        const timeoutId = setTimeout(() => reject(new Error('timezone timeout')), timeoutMs);
        Promise.resolve()
            .then(() => run())
            .then(value => {
...
export class TimezoneProvider {
    ...
    async _loadCityLookup() {
        if (this._destroyed)
            throw new Error('timezone canceled');
    ...
    _startFallbackLookup(token) {
        if (this._destroyed || this._inflight.has(token))
            return;

        const run = runWithTimeout(() => this._resolveCityTimezone(token), this._timeoutMs)
            .then(payload => {
                if (this._destroyed)
                    return;
    ...
    getResults(query, mode = 'all') {
        if (this._destroyed || (mode !== 'all' && mode !== 'timezone'))
            return [];
