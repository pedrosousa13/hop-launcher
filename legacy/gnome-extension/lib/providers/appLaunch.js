function resolveBoolFlag(owner, value) {
    if (typeof value === 'function') {
        try {
            return Boolean(value.call(owner));
        } catch (_error) {
            return false;
        }
    }
...
function canUseWindow(window) {
    if (!window)
        return false;
    if (resolveBoolFlag(window, window.skip_taskbar) || resolveBoolFlag(window, window.is_skip_taskbar))
        return false;
